...
# Quick fix blender 5.0
#replacing all 'action.fcurves' by 'get_action_fcurves(action)'
# and all 'if/while len(action.fcurves) == 0:' by 'fcurves = get_action_fcurves(src_arm.animation_data.action)
#                                            if/while not fcurves or len(fcurves) == 0:
#                                               return

def get_action_fcurves(action):
    """Return fcurves safely across Blender 4/5."""
    if action is None:
        return None
    # Blender <4.1 style
    ...
    if fc:
        return fc
    # Blender 5: actions may use animation_data instead of fcurves
    if hasattr(action, "animation_data") and action.animation_data:
        sub_action = getattr(action.animation_data, "action", None)
        if sub_action:
            return getattr(sub_action, "fcurves", None)
    return None
...
def _zero_out():
    ...
    if action:
        ...
        while fcurves and len(fcurves):
            # remove the first fcurve
...
def _import_anim(src_arm, tar_arm, import_only=False):
    ...
    fcurves = get_action_fcurves(src_arm.animation_data.action)
    if not fcurves or len(fcurves) == 0:
        return
